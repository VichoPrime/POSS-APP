                            ...
                            <div key={index} className="row mb-2 align-items-end">
                              ...
                              <div className="col-md-4">
                                <label className="form-label">
                                  {(() => {
                                    const selectedProduct = products.find(p => p.id === producto.id);
                                    return selectedProduct?.unit_type === 'peso' ? 'Peso (kg)' : 'Cantidad';
                                  })()}
                                </label>
                                <input
                                  type="text"
                                  className="form-control"
                                  value={producto.cantidad === 0 || producto.cantidad === '' ? '' : producto.cantidad}
                                  onChange={(e) => {
                                    const selectedProduct = products.find(p => p.id === producto.id);
                                    ...
                                    // Permitir: vacío, números con o sin decimales (0, 0., 0.5, 1, 1., 1.5, .5)
                                    // La regex permite: punto opcional al inicio, seguido de dígitos, punto/coma opcional, más dígitos
                                    if (inputValue === '' || /^\.?\d*[,.]?\d*$/.test(inputValue)) {
                                      // Guardar el valor normalizado
                                      if (normalizedValue === '' || normalizedValue === '.') {
                                        updateProductCombo(index, 'cantidad', normalizedValue as any);
                                      } else {
                                        // Guardar como número si es completo, o como string si está incompleto (ej: "1." o "0.")
                                        const parsed = parseFloat(normalizedValue);
                                        const valueToSave = normalizedValue.endsWith('.') || normalizedValue.endsWith(',') 
                                          ? normalizedValue 
                                          : (isNaN(parsed) ? normalizedValue : parsed);
                                        updateProductCombo(index, 'cantidad', valueToSave as any);
                                      }
                                    }
                                  }}
                                  onBlur={(e) => {
                                    const selectedProduct = products.find(p => p.id === producto.id);
                                    let value = producto.cantidad;
                                    
                                    // Convertir string a número al perder el foco
                                    if (typeof value === 'string') {
                                      const numValue = parseFloat(value);
                                      if (isNaN(numValue) || numValue === 0) {
                                        updateProductCombo(index, 'cantidad', selectedProduct?.unit_type === 'peso' ? 0.1 : 1);
                                      } else {
                                        updateProductCombo(index, 'cantidad', numValue);
                                      }
                                  ...
                                  placeholder={(() => {
                                    const selectedProduct = products.find(p => p.id === producto.id);
                                    return selectedProduct?.unit_type === 'peso' ? 'Ej: 0.5 o 0,5' : 'Ej: 2';
                                  })()}
                                />
                              </div>
                          ...
                          <div className="col-md-6">
                            <div className="mb-3">
                              ...
                              <input
                                ...
                                placeholder="Ej: 3 (o 0.5 para peso)"
                                min="0.001"
                                step="0.001"
                              />
                              <div className="form-text">
                                Cantidad mínima de productos (unidades o kg) para aplicar descuento
                              </div>
                            </div>
                          </div>
                          <div className="col-md-6">
