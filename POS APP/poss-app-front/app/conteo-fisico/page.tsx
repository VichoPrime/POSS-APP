...
export default function PhysicalInventoryPage() {
  ...
  const [sessionInfo, setSessionInfo] = useState<InventorySession | null>(null);
  ...
  const loadSessionData = async (sessionId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // Buscar información de la sesión en la lista de sesiones
  ...
  // Efecto para actualizar sessionInfo cuando cambian las sesiones o currentSession
  useEffect(() => {
    if (currentSession && sessions.length > 0) {
      const sessionInfo = sessions.find(s => s.session_id === currentSession);
      setSessionInfo(sessionInfo || null);
    }
  ...
  const applyAdjustments = async (adjustAll: boolean = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        let mensaje = data.message || 'Ajustes aplicados exitosamente';
        
        if (data.ajustes && data.ajustes.length > 0) {
          mensaje += `\n\nProductos ajustados: ${data.ajustes.length}`;
          if (data.ajustes.length <= 5) {
            data.ajustes.forEach((ajuste: any) => {
              mensaje += `\n- ${ajuste.article_title}: ${ajuste.stock_anterior} → ${ajuste.stock_nuevo}`;
            });
          }
        }
        
        if (data.sin_cambios && data.sin_cambios.length > 0) {
          mensaje += `\n\nProductos sin cambios: ${data.sin_cambios.length}`;
        }
        ...
        let errorMessage = 'Error al aplicar ajustes de inventario';
        try {
          const error = await response.json();
              ...
              <div className="card-body">
                <div className="row">
                  ...
                  <div className="col-md-4 text-end">
                    {sessionInfo && sessionInfo.estado === 'Completado' && (
                      <div className="alert alert-info mb-2 p-2">
                        <small>
                          <strong>Sesión Completada:</strong> No se pueden realizar más modificaciones.
                        </small>
                      </div>
                    )}
                    <button
                      ...
                      disabled={selectedItems.length === 0 || loading || sessionInfo?.estado === 'Completado'}
                      title={sessionInfo?.estado === 'Completado' ? 'Esta sesión está completada' : ''}
                    >
                    ...
                    <button
                      ...
                      disabled={!stats || stats.con_diferencias === 0 || loading || sessionInfo?.estado === 'Completado'}
                      title={sessionInfo?.estado === 'Completado' ? 'Esta sesión está completada' : ''}
                    >
                        ...
                        <tr key={`conteo-${conteo.id}-${index}`} className={conteo.tiene_diferencia ? 'table-warning' : ''}>
                          <td>
                            <input
                              ...
                              disabled={!conteo.tiene_diferencia || sessionInfo?.estado === 'Completado' || conteo.estado === 'ajustado'}
                            />
                          </td>
                          <td>
                          ...
                          <td>
                            {conteo.estado === 'contado' && sessionInfo?.puede_editar && (
                              <button
                                ...
                                disabled={sessionInfo?.estado === 'Completado'}
                                title={sessionInfo?.estado === 'Completado' ? 'Esta sesión está completada' : ''}
                              >
                                Editar
                              </button>
                            ...
                            {conteo.estado === 'pendiente' && sessionInfo?.puede_editar && (
                              <button
                                className="btn btn-sm btn-primary"
                                onClick={() => {
                                  const cantidad = prompt('Cantidad física contada:', conteo.cantidad_sistema.toString());
                                  if (cantidad !== null) {
                            ...
                            {sessionInfo?.estado === 'Completado' && conteo.estado === 'pendiente' && (
                              <span className="text-muted">
                                <small>No contado</small>
                              </span>
                            )}
                          </td>
                        </tr>
                  ...
                  <tbody>
                    {sessions.map((session, index) => (
                      <tr key={`session-${session.session_id}-${index}`}>
                        ...
                        <td>
                          <span className={`badge ${
                            session.estado === 'Completado' ? 'bg-success' :
                            session.estado === 'Parcialmente Completado' ? 'bg-warning' :
                            session.estado === 'En Progreso' ? 'bg-primary' : 'bg-secondary'
                          }`}>
                            {session.estado}
                          </span>
                        </td>
                        <td>
                          ...
                          >
                            {session.puede_editar ? 'Editar' : 'Ver Detalles'}
                          </button>
                        </td>
                      </tr>
                    ))}
