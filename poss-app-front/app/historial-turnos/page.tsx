...
import GraficosModal from '@/components/GraficosModal';
...
export default function HistorialTurnos() {
  ...
  // Estados para mostrar notas
  const [showNoteModal, setShowNoteModal] = useState(false);
  const [selectedNote, setSelectedNote] = useState('');
  const [selectedSaleTicket, setSelectedSaleTicket] = useState('');
  ...
  const loadTurnos = async () => {
    try {
      ...
      const response = await api.get(`/turnos-historial?${params.toString()}`);
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const response = await api.get(`/turnos-historial/excel?${params.toString()}`, {
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', `reporte_turnos_${fechaInicio}_${fechaFin}.xlsx`);
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const response = await api.get(`/turnos-historial/pdf?${params.toString()}`, {
        responseType: 'blob'
      });
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `reporte_turnos_${fechaInicio}_${fechaFin}.pdf`);
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (error) {
  ...
  const mostrarNota = (nota: string, ticketNumber: string) => {
    setSelectedNote(nota);
    setSelectedSaleTicket(ticketNumber);
  ...
  return (
    <PermissionGuard permission="can_view_shift_history">
                            ...
                            <td>
                              {venta.nota ? (
                                <button
                                  ...
                                  onClick={() => mostrarNota(venta.nota!, venta.ticket_number)}
                                  title="Ver nota de la venta"
                                >
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  ...
                  Nota de la Venta {selectedSaleTicket}
                </h5>
                <button 
                  type="button" 
                  className="btn-close"
