...
export function useCart() {
  ...
  const addToCart = (product: Product, customQuantity?: number) => {
    ...
    let quantityToAdd = customQuantity || 1;
    
    // Si es un producto por peso, redondear a 2 decimales
    if (product.unit_type === 'peso') {
      quantityToAdd = Number(quantityToAdd.toFixed(2));
    }
  ...
  const updateQuantity = (index: number, newQuantity: number) => {
    ...
    // Redondear a 2 decimales para productos por peso
    const roundedQuantity = item.unit_type === 'peso' ? Number(newQuantity.toFixed(2)) : newQuantity;

    if (roundedQuantity > item.stock) {
      alert(`⚠️ Stock insuficiente\n\nProducto: ${item.title}\nStock disponible: ${item.stock}${item.unit_type === 'peso' ? 'kg' : ' unidades'}\nCantidad solicitada: ${roundedQuantity}${item.unit_type === 'peso' ? 'kg' : ' unidades'}`);
      return; // No actualizar la cantidad, mantener la actual
    }

    const newCart = cart.map((item, i) =>
      i === index ? { ...item, quantity: roundedQuantity } : item
    );
    saveCart(newCart);
  };
